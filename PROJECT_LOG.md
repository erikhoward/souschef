...
- **RESOLVED 2026-08-07 — `telegram.Deps.Client` is now the `telegram.API` interface.** It was a concrete `*Client` hardcoded to `api.telegram.org`, which made `handleSearch` / `handleRecent` / `handleCallback` and the capture flow impossible to exercise without a live token — the largest untested surface in the codebase, and where two of the three must-fix defects from the Milestone 1 final review turned out to live. `API` declares only the seven methods the bot uses, with a compile-time assertion keeping `*Client` conforming. The package went from 18 tests to 41, covering: search returning tappable results with no id in any label or body, callbacks always being answered (including unknown actions and deleted ideas), unauthorised chats getting zero interactions, capture sending one message and editing it, failed enrichment showing the verbatim error with Retry, voice capture and its failure path, and menu-sync behaviour. Test teeth were verified by mutation rather than assumed — leaking an id into a button label and dropping the unknown-callback answer each turn the relevant test red.
